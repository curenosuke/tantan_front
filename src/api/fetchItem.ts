export default async function fetchItem(canvasData: {
  ...
  try {
    ...
    const userResponse = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/auth/me`, {
      method: 'GET',
      credentials: 'include',
    })
    ...
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/projects`, {
      method: 'POST',
      headers: {
