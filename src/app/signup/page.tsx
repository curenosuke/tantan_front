...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_ENDPOINT}/api/signup`, {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8">
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          または{' '}
          <Link href="/login" className="font-medium text-[#FFBB3F] hover:text-orange-600 transition-colors">
            ログインはこちら
          </Link>
        </p>
      </div>
          ...
          <div className="bg-white rounded-lg p-8 max-w-md mx-4 shadow-xl">
            <div className="text-center">
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-[#FFBB3F] to-orange-500 hover:from-orange-500 hover:to-[#FFBB3F] text-white font-medium py-3 px-4 rounded-lg transition-all duration-300 transform hover:scale-105 hover:shadow-lg shadow-md"
              >
                ログイン画面へ
              </button>
            </div>
